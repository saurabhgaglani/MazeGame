...
/** Prints the maze*/
void printmaze(char** mazeToBePrinted)
{
    int x = 0;
    int y = 0;
    for(int i = 0; i < 10; i++)
    {
        for(int j = 0; j< 20; j++)
        {
            printf("%c", mazeToBePrinted[i][j]);
        }
        printf("\n");
    }
}

/** Recieves character from user without echoing it */
...
/** Moves the character*/
void movethecharacter(char** playerControl)
{
    int xAxis = 1; //Hardcoding it in because wherex() and wherey() not working without conio.h
    int yAxis = 4;
    char player = '@';
    char key = 0;
    int pcc = 0;
    ...
    while(key != '\n')
    {
      ...
      switch (key) 
      {
        case keyUp: gotoxyor(xAxis, yAxis);
        printf(" ");

        if(playerControl[pcr-1][pcc] == 'w')
        {
            int check = pcr +1;
            if(check >= 10 && potions >= 2)
            {
                gotoxyor(20, 50);
                printf("CONGRATULATIONS!! YOU HAVE WON!! TRY AGAIN IN A DIFFERENT MAZE\n\n");
                key = '\n';
            }

            else if(check >= 10 && potions < 2)
            {
                gotoxyor(20, 50);
                printf("You need to collect all potions,  please try again!");
            }
        ...
        {
            if(playerControl[pcr-1][pcc] == '#')
            {
                potions += 1;
                playerControl[pcr-1][pcc] = ' ';
                gotoxyor(20, 20);
                printf("Potions = %i",  potions);
            }
            yAxis -= 1;
            gotoxyor(xAxis, yAxis);
            pcr -= 1;
        ...
        case keyDown: gotoxyor(xAxis, yAxis);
        printf(" ");
        ...
        case keyLeft: gotoxyor(xAxis, yAxis);
        printf(" ");

        if(playerControl[pcr][pcc-1] == 'w')
        {
            int check = pcr +1;
            if(check >= 10 && potions >= 2)
            {
        ...
        {
            if(playerControl[pcr][pcc-1] == '#')
            {
                potions += 1;
                playerControl[pcr][pcc-1] = ' ';
                gotoxyor(20, 20);
                printf("Potions = %i",  potions);
            }
            xAxis -= 1;
        ...
        case keyRight: gotoxyor(xAxis, yAxis);
        printf(" ");
...
void main()
{
    ...
    //Hard coding these values because scanf was passing '\n' into movethecharacter. Help required here.
	recMaze = generate_maze(10,10,1,1);
    printmaze(recMaze.a);
    movethecharacter(recMaze.a);
}
